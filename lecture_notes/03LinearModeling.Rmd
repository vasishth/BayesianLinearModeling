...
2. The second or third most watched talk in the history of TED talks is by Amy Cuddy, some 49 million views. Watch the talk first: to-do

Also read the original paper: to-do.
...
A typical approach would be to do a 
two-sample t-test:
...
There seems to be no evidence for power posing. 
...
library(rstan)
library(parallel)
rstan_options(auto_write=TRUE,results="hide")
options(mc.cores=parallel::detectCores())

## cauchy priors on beta params:
#output <- stanc("linearmodel.stan")
## normal priors N(0,20) on beta params:
output <- stanc("StanModels/linearmodel_normalpriors.stan")

fit <- stan(file='StanModels/linearmodel_normalpriors.stan', data=dat,
