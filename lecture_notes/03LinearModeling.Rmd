...
\item
The term \texttt{control} refers to some optional control parameters for the sampler, such as \texttt{adapt\_delta}, \texttt{max\_treedepth}, and so forth, to be discussed later.
\end{enumerate}
...
always a good idea, for interpretability and for computational reasons. See @SchadEtAlcontrasts for details on this point.
...
previous literature. Sometimes this requires a meta-analysis. See @JaegerEtAl2017, @NicenboimRoettgeretal for examples. For examples of the use of this prior knowledge, see  @NicenboimEtAl2016NIG.
...
```{r lognormal, fig.height=2,fig.width=3.5, fig.show='hold', message=FALSE, warning=FALSE,fig.cap="\\label{fig:logndemo}The log-normal distribution."}
mu <- 6
sigma <- 0.5
N <- 100000
...
```{r fig.cap="\\label{fig:logexp}Back-transforming to milliseconds."}
...
ggplot(df, aes(x=Intercept,y=ms)) + geom_point() +
  scale_y_continuous("Difference in milliseconds")
```
...
We can summarize the posterior and do inference as discussed in Example 1. If we want to talk about the effect estimated by the model, we summarize the posterior of $\beta$ in the following way: $\hat\beta = `r mean(beta_samples)`$, 95% CrI = $[ `r beta_low` , `r beta_high` ]$, $P(\beta >0)  \approx `r mean(beta_samples > 0)`$
...
1. Load the data-set beauty.txt. This data-set shows beauty ratings and the teaching evaluation score of professors in a US university. The data are from @GelmanHill2007.
...
A beauty level of 0 means average beauty. Using a Bayesian linear model, carry out a full analysis to evaluate the question: does the physical attractiveness (beauty level) of the professor lead to a better teaching evaluation? 
- Plot the data and look at the distributions of the data.
- Define a model in Stan to answer the question.
- Do a fake-data simulation to check whether the model recovers the parameters.
- Once satisfied with the model, fit the model and display the posteriors.
- Carry out a posterior predictive check to determine whether the model makes reasonable predictions.
- What would you conclude about the effect of beauty on evaluations?

2. The second most watched talk in the history of TED talks is by Amy Cuddy, some 32 million views. Watch the talk first: to-do

Also read the original paper: .

The claim is that raising your arms for two minutes will increase your testosterone, improving your performance in, e.g., job interviews. We are going to evaluate this claim based on Cuddy's data. Incidentally, the first author of the Cuddy paper has disavowed the paper: . 
...
Then carry out the following exploration of the data.
...
Means ignoring gender: Figure \ref{fig:cuddyplot} is the rough and ready version of fig 3 of the paper.
...
```{r fig.cap="\\label{fig:cuddyplot}Recreation of Figure 3 in the Cuddy power posing study."}
...
One could do a two-sample t-test as shown below. Note the high variability in the change in testosterone in low and high pose subjects. There were many cases where testosterone went down post high-power pose. 
...
```{r fig.cap="\\label{fig:testchange}Change in testosterone in low and high power pose conditions."}
## graphical summary of distributions:
op<-par(mfrow=c(2,2),pty="s")
boxplot(lowdiff$diff,main="Low",ylim=c(-50,50),
        ylab="change in testosterone")
boxplot(highdiff$diff,main="High",ylim=c(-50,50),
        ylab="change in testosterone")
barplot(lowdiff$diff,main="Low",ylim=c(-50,50),
        ylab="change in testosterone")
barplot(highdiff$diff,main="High",ylim=c(-50,50),
        ylab="change in testosterone")
```
...
There is actually no evidence of power posing. 
Now, refit the model using various predictors: 
- the initial testosterone value
- the initial and final cortisone (?) levels 
- the gender of the subject. 

The question we ask here is, is post-treatment
testosterone higher for subjects exposed to high vs low power, controlling for these variables?
...
summary(m0<-lm(testm2~ctestm1+chptreat+cortm1+
                 cortm2+female,datc))
summary(m0a<-lm(testm2~ctestm1+cortm1+cortm2+
                  female,datc))
...
Fit the above model m2 using brms.
