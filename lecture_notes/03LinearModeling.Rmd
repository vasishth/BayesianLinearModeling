...
```{r, reading_noreading}
noreading_data <- read.table(header = F,"data/button_press.dat",encoding="latin1")
...
The summary displayed below includes means,  standard  deviations  (`sd`),  quantiles,  Monte
...
Graphical summary of posterior distributions:

```{r}
stanplot(m,type="hist")
```

Trace plots:

```{r}
stanplot(m,type="trace")
```
...
m_fake<-brm(RT~1,fake_data,prior = priors,
       iter = 2000, chains = 4,family = gaussian(), 
       control = list(adapt_delta = 0.99))
```
...
summary(m_fake)
```
You should also check the trace plots to confirm that the model converged.
...
```{r}
hist(RT)
stanplot(m_fake,type="hist",pars="b_Intercept")
```
