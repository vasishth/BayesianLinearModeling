...
In essence, we integrate out the parameters. Here is one way to do it in R:
  ...
  - Take a sample from each of the priors
  - Generate data using those samples
  - Repeat until you have some data
...
hist(y_sim$y_sim[,1],main="Prior predictive distribution",xlab="y_sim",freq=FALSE)
```
...
op<-par(mfrow=c(1,2),pty="s")
hist(posteriors$mu,main=expression(paste("posterior for ",mu)),freq=FALSE,xlab="")
abline(v=400)
hist(posteriors$sigma,main=expression(paste("posterior for ",sigma)),freq=FALSE,xlab="")
abline(v=125)
```
...
An alternative way to plot is shown in Figure \ref.
...
```{r lognormal, fig.height=2,fig.width=3.5, fig.show='hold', message=F, warning=F,fig.cap="\\label{fig:logndemo}The log-normal distribution."}
mu <- 6
sigma <- 0.5
N <- 100000
...
\begin{itemize}
\item Basic brms and Stan syntax.
...
\item Model evaluation using prior and posterior predictive checks.
