...
\item The largest sources of variance are the subject intercepts, slopes, and the residual standard deviation. Look at sd\_subject parameters, and sigma.
\item The by-item variance components are relatively small. Look at sd\_item parameters.
...
Incidentally, the frequentist lmer model would fail entirely to estimate the correlation parameters, but it would *look* like everything is fine---if you don't know what properties a variance covariance matrix must have  in order to not be *degenerate*.

```{r results="hide"}
library(lme4)
m_gglmer<-lmer(log(rawRT)~so + (1+so|subject)+(1+so|item),gge1crit)
```
...
The effect of RC type looks excitingly significant and researchers will stop looking as soon as they find an absolute t-value larger than 2.

But the by-item variance matrix is degenerate because the matrix is not invertible. We see this by computing its determinant, and find that it's 0, which entails that it is not invertible:

```{r}
itemvar<-matrix(c(0.001719,0.04146*0.04431,
                  0.04146*0.04431,0.001963),ncol=2)
det(itemvar) ## effectively 0 => non-invertible
```

This is one reason why one shouldn't blindly fit maximal models with lmer; see the discussion in @barr2013 and @BatesEtAlParsimonious. In Bayesian models, the argument can be made that one can always fit a maximal model. This is so because the priors will always regularize the posterior (push it closer to zero) if there isn't enough data, and the posterior will have high uncertainty, which alerts the researcher that not enough information is present in the data for estimating the parameter in question. 
\end{itemize}
...
Figure \ref{fig:ggsubjint} shows to-do
...
mns <- colMeans(subjint)
subjint<-subjint[,order(mns)]
mcmc_areas(subjint)
```

#### By subject slope adjustments

Figure \ref{fig:ggsubjslope} shows to-do
...
mns <- colMeans(subjslope)
subjslope<-subjslope[,order(mns)]
...
Suppose that, based on these data and this model, we want to claim that there is a mean OR processing cost in English (this is what I just showed you above; it makes no sense as a question, but we will suspend disbelief for now). In order to make a discovery claim, we need to understand the **true discovery rate** of this effect. In the frequentist world, this would be the statistical power, the probability of your detecting an effect if there is in fact one.
