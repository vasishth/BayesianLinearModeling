...
1. Load the following two data-sets: gibsonwu2012data.txt, which is a data-set from @gibsonwu. This is self-paced reading data from Chinese relative clauses. The research question is whether Chinese is like English (does Chinese show an OR processing cost or not). The replication data, gibsonwu2012datarepeat.txt, is from @VasishthetalPLoSOne2013. This data uses the same items as in Gibson and Wu's study, but it has new subjects.

To load the replication data, run the following code:
...
For each of the two data-sets, do the following:
  ...
  - First re-read \ref{maximal}. Write down in mathematical notation the maximal hierarchical linear model, with varying intercepts and slopes for subjects and items in order to investigate the effect of relative clause type on reading time. Assume a Log-Normal likelihood. 
  ...
  - Carry out a posterior predictive check. Does the model's predicted data approximately match the observed data?
...
m_gw<-brm(rt~so + (1+so|subj) + (1+so|item),headnoun,family=lognormal(),
    prior=priors)
summary(m_gw)
```
...
Our theoretical interest is in determining whether relative clause type and distance influence reading time, and whether there is an interaction between these two factors. We will use brms to determine the posterior probability distribution of the fixed effect $\beta _1$ for relative clause type, the fixed effect $\beta _2$ for distance, and their interaction $\beta _3$.

Fit a varying intercepts, varying slopes model (maximal model) to this data-set. 
The model you should fit is:
...
```
  ...
  - First re-read \ref{rawscale}. Extract the posteriors of the effect of RCType, dist, and int, and then transform them to the raw ms scale. Display the posterior distributions on the ms scale.   
