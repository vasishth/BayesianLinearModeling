...
\begin{enumerate}
\item Given data, specify a \textbf{likelihood function}.
...
In probability, we are given a probability density or mass function f(x)  (see below), and parameters, and we can deduce the probability of one or more events occurring.

In statistics, we are given a collection of events, and we want to discover the parameters that produced them (assuming f(x) is the pdf that generated the data). The classical frequentist approach is:
...
For example, for reaction times, we might assume a random variable $X_i$ that has a normal distribution $N(0,\sigma^2)$ (the null hypothesis) as its probability density function. We compute the most likely parameter value that generated the data, the mean of the random variable,\footnote{This is the maximum likelihood estimate.} and compute the probability of observing a value like the mean or something more extreme given the null hypothesis. We can write this statement in short-hand as $P(data\mid parameter)$.
...
These are provided only for reference; you won't need to actively use them in this course.
