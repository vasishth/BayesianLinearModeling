...
This document and all associated material are provided under a [Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License](https://creativecommons.org/licenses/by-nc-sa/4.0/). The materials are available from: 

  - [OSF](https://osf.io/jhu47/)
  - [github](https://github.com/vasishth/BayesianLinearModeling)

**Acknowledgments**: A few things have been borrowed from the linear modeling and hierarchical linear modeling chapters in 
...
We see this in the way statistics is routinely abused in psycholinguistics, psychology, and linguistics, among other areas. Some examples from psycholinguistics are mentioned alongside each point: 
  ...
  - decide that an effect is ``reliable'' if $p<0.05$ in a severely underpowered study; for examples, see @JaegerEtAl2017, @VasishthMertzenJaegerGelman2018.
  - inconrrectly argue that the null is true when $p>0.05$, or arguing that the null is false when is a bit over $p>0.05$, but the researcher wants desperately that p be less than 0.05;  for discussion, see @VasishthNicenboimStatMeth, @NicenboimVasishth2016.
  - flexibly analyze data to get the result desired: analyze many dependent measures and choose whichever result you like, ignoring the rest; see @MalsburgAngele2016.
 ...
 - argue for a significant interaction after showing a significant effect in one pair of conditions and no significant effect in another pair [@nieuwenhuis2011erroneous].
...
In the past, I have made these mistakes too. For examples of mistakes listed above from my own work, see @vasishthlewisLanguage05, @vasishth:phdbook. 
...
If you find typos or outright errors in this document, please open an issue [here](https://github.com/vasishth/BayesianLinearModeling). Alternatively, if you know how to, you can alternatively [fork and make a pull request](https://help.github.com/articles/fork-a-repo/).
...
use Bayes' theorem to quantify uncertainty about a scientific question of interest. In order to understand the methodology, some passive understanding of the following topics needs to be in place:
  ...
  - The theory of random variables
    - the distinction betweeen the probability density/mass function $f(x)$ vs cumulative distribution function $F(x)$
    - inverse CDF, $F^{-1}(x)$
...
Without a clear understanding of these concepts, generalized confusion is an inevitable outcome. 
That is why it is so important to expend some energy in understanding these concepts. 
...
### The normalization constant in pdfs
...
Why are we going through these basic results? As Betancourt puts it (see [here](https://www.youtube.com/watch?v=VnNdhsm0rJQ), at minute 9:55): ``Bayesian computations reduce to (computing) expectations and, consequently, to integration.'' We will be computing expectations a lot in this course, using software. We will never have to do this by hand because we will be working in high-dimensional spaces and will have no choice but to use numerical approximations.
...
```{r}
# matrix z_u
z_u <- matrix(c(z_u0,z_u1),ncol=N_subj,byrow=T)
...
\begin{enumerate}
\item 
We plan to toss a fair coin three times. 
\begin{itemize}
\item What is the theoretical probability of obtaining
\begin{itemize}
\item 0 heads
\item 1 heads
\item 2 heads
\item 3 heads
\end{itemize}
\item What is the theoretical probability of obtaining \begin{itemize}
\item 0 or 1 heads
\item 0,1, or 2 heads
\item 0-3 heads?
\end{itemize}
\end{itemize}
\item 
Toss a coin 10 times and count the number of heads, Compute, using pbinom, the theoretical probability of getting the total numbers of heads you got, assuming that the coin is fair.

Hint: given sample size $n$, your assumed probability of a heads $prob$, and the number of heads you got $x$, the pbinom function delivers $P(X \leq x)$, the probability of getting $x$ heads or less. In other words, pbinom is the cumulative distribution function.
...
Based on what everyone in the class finds, we can write down the number of cases we have of each possible outcome (0 heads, 1 heads, \dots, 10 heads), along with the theoretical probability. 
...
\item A random variable X comes from a LogNormal distribution with mean 6 log milliseconds and standard deviation 0.1 log ms. Plot this distribution. What is the mean on the ms scale? 
\item A random variable X comes from a Gamma distribution with mean 100 and standard deviation 10. Find the shape and rate parameters of the Gamma distribution.

\item 

Let $X$ and $Y$ be discrete random variables with values of $p_{X,Y}(x,y)$ as shown below:
...
\end{tabular}
\end{center}
\caption{The joint PMF for $X$ and $Y$.}\label{q5table}
\end{table}

\begin{enumerate}
\item Find the conditional distribution of X given each of the possible values of Y.
\item 
Find the marginal distribution of X. Then calculate $E[X]$.
\end{enumerate}


\item Re-read \ref{chol} first. Generate two correlated random variables of length 100 using the Cholesky factorization. Let the correlation be $-0.7$. Display the correlation in a plot.

\item Generate 1000 data points from a Gamma distribution with mean 100 and sd 10. Then, use the data and the optim function to get parameter estimates of the Gamma distribution. Does the output of the optim function match the theoretical values?
\end{enumerate}  
