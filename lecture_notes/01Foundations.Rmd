...
Given a correlation matrix $\boldsymbol{\rho_u}$, we can obtain a square root of the matrix. If we multiply the lower triangular matrix $\mathbf{L_u}$ with its transpose, we get the correlation matrix:
...
Step 1.  We generate uncorrelated values that will be associated with random variable $u$ from a $Normal(0,1)$. In our case we have $u_0$ and $u_1$, so
    we will generate:
...
Step 3. The last step is to scale the previous matrix to the desired standard deviation. We define the diagonalized matrix $diag\_matrix(\tau_u)$ as before:
...
This will be useful when fitting hierarchical linear models: we will define a prior on the Cholesky factor of the correlation matrix, and then assemble the variance-covariance matrix as shown above.
...
Based on what everyone finds, we can write down the number of cases we have of each possible outcome (0 heads, 1 heads, \dots, 10 heads), along with the theoretical probability. 
...
\item Generate two correlated random variables using the Cholesky factorization. to-do
Visualize the bivariate distrn.
\end{enumerate}  
