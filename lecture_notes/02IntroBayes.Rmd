...
\begin{enumerate}
\item \textbf{Optional: if you know the basics of  integration}.
...
\end{enumerate}
\item
Plot the priors, likelihoods, and posterior distributions in the four Beta-Binomial cases discussed in this chapter.

```{r binomexample1,echo=FALSE,include=FALSE}
###lik:
plot(function(x) 
  dbeta(x,shape1=46,shape2=54),0,1,
              ylab="",xlab="X",col="red")
...
\item
Consider normally distributed data with unknown mean but known variance $\sigma^2$. 
Before getting into the exercise, recall the idea of the \textit{likelihood function}.

Suppose $X_1,\dots,X_n$ is a random variable that comes from a normally distributed population with mean $\mu$ and variance $\sigma^2$. \textbf{In this exercise, we will talk about the Normal distribution in terms of the mean and variance, not standard deviation}. 
...
The last line $f(X_1=x_1,X_2=x_2,\dots,X_n=x_n;\theta)$ means: ``the joint probability of the values $x_1,\dots,x_n$ given a specific value for the parameter(s) $\theta$.'' Here, $\theta$ is the vector $<\mu>$. If $\sigma$ were unknown, $\theta$ would be $<\mu,\sigma>$.
...
This is the \textit{likelihood function}.
...
So let us suppose we have independent and identically distributed data $x=\{x_1,\dots,x_n\}$, where $X_i \sim Normal(\mu,\sigma^2)$. As mentioned above, assume that $\sigma^2$ is known as above, but $\mu$ is unknown.  

It follows that the pdf for random variable representing any one data point is $Normal(\bar{x},\sigma^2)$. 
...
it is straightforward to derive analytically (proof omitted, but see @Lynch2007) that the posterior will be $Normal(m*,v*)$, where
...
Let the data be: 15.7,14.7,16.1,16.2,15.1. Variance ($\sigma^2$) is known to be 0.6.
Let the prior be  N(m=14,v=1).
We will work out the posterior distribution using the formula.
...
What role does sample size n play in the computation of the posterior? When sample size is increased in the above example, 
