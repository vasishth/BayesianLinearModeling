...
let $f(x) = \frac{1}{40} (2x + 3)$, with $0<x<5$. We have to draw a number from the uniform distribution and then solve for z, which amounts to finding the inverse function:
...
Gibbs sampling is a very commonly used method in Bayesian statistics.
...
Using the results discussed in the Foundations chapter, it is possible to analytically work out the conditional distributions:
...
A good reference explaining HMC is @neal2011mcmc. However, this paper is technically very demanding. More intuitively accessible introductions are available via  Michael Betancourt's home page: https://betanalpha.github.io/.
...
This book assumes no calculus or linear algebra knowledge.

  - Kruschke, J. (2014). Doing Bayesian data analysis: A tutorial with R, JAGS, and Stan. Academic Press. This book is specifically designed for a psychology audience. 

  - Lynch, S. M. (2007). Introduction to applied Bayesian statistics and estimation for social scientists. Springer Science & Business Media. This book assumes knowledge of calculus and linear algebra.
